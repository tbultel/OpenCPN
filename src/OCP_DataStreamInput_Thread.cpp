...
OCP_DataStreamInput_Thread::OCP_DataStreamInput_Thread(
    DataStream *Launcher, wxEvtHandler *MessageTarget, const wxString &PortName,
    const wxString &strBaudRate, dsPortType io_select) {
...
bool OCP_DataStreamInput_Thread::OpenComPortPhysical(const wxString &com_name,
                                                     int baud_rate) {
  try {
      ...
      if (nl_found) {
        ...
        while ((*tptr != 0x0a) && (tptr != put_ptr)) {
          *ptmpbuf++ = *tptr++;
          ...
          wxASSERT_MSG((ptmpbuf - temp_buf) < DS_RX_BUFFER_SIZE,
                       (const wxChar *)L"temp_buf overrun1");
        }
        if ((*tptr == 0x0a) && (tptr != put_ptr))  // well formed sentence
        {
          *ptmpbuf++ = *tptr++;
          ...
          wxASSERT_MSG((ptmpbuf - temp_buf) < DS_RX_BUFFER_SIZE,
                       (const wxChar *)L"temp_buf overrun2");
